...
SparkMaxConfig& DrivingConfig() {
    ...
    drivingConfig.SetIdleMode(SparkBaseConfig::IdleMode::kBrake)
        .SmartCurrentLimit(50); // done
    drivingConfig.encoder
        .PositionConversionFactor(drivingFactor)       // done   // meters
        .VelocityConversionFactor(drivingFactor / 60.0); // done // meters per second
    drivingConfig.closedLoop
        .SetFeedbackSensor(ClosedLoopConfig::FeedbackSensor::kPrimaryEncoder) // done
